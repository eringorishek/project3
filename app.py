...
@app.route('/medianhousing')
def get_median_housing_data():
    page_sanitized = json.loads(json_util.dumps([i for i in med_housing.find({})]))
    #pprint(list([i for i in med_housing.find({})]))
    return page_sanitized

@app.route('/merged_housing')
def merged_housing():
    #load in data
    geo_data2 = json.loads(json_util.dumps([i for i in db.geo_data.find({})]))
    median_housing = json.loads(json_util.dumps([i for i in med_housing.find({})]))
    # create dictionary & loop through .geojson and .json to merge at zipcode
    all_zipcodes = {
        #zipcode: #price_change
    }
    for key in median_housing[0]['RegionName']:
        all_zipcodes[str(median_housing[0]['RegionName'][key])] = median_housing[0]['price_change'][key]
    for feature in geo_data2[0]['features']:
        if feature['properties']['geoid10'] in all_zipcodes:
            feature['properties']['housing'] = all_zipcodes[feature['properties']['geoid10']]
        else: 
            print(feature['properties']['geoid10'])
    return jsonify(geo_data2)
